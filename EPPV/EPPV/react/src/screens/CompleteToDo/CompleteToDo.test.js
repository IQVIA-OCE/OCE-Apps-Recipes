...
import { Autocomplete, DarkTheme, Provider, Select } from 'apollo-react-native';
...
describe('CompleteToDo screen', () => {
  ...
  it('should render properly and call save form', async () => {
    ...
    const { container, getByTestId } = await waitFor(() =>
      render(<CompleteToDo route={{ params: params }} />)
    );
    await act(() => promise);

    const formikElement = await container.findByType(Formik);
