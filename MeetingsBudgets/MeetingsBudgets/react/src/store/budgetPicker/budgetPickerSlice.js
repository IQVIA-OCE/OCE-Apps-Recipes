...
export const saveBudget = createAsyncThunk(
  ...
  async (budget, { getState, dispatch, rejectWithValue }) => {
    try {
      ...
      const [[response], ignoredConditions] = await budgetsApi.saveBudget(
        budget,
        meeting.id
      );
      dispatch(removeSelectedBudget(budget));

      const isNotSupported = ignoredConditions.isNotSupported.length;
      const isTooLong = ignoredConditions.isTooLong.length;

      if (isNotSupported || isTooLong) {
        warning = localized(
        ...
        ).replace('%1$@', String(isNotSupported + isTooLong));

        if (isNotSupported) {
          warning += ` ${isNotSupported} conditions are not supported.`;
        }
        if (isTooLong) {
          warning += ` ${isTooLong} conditions are too long.`;
        }
      } else {
        success = localized(
      ...
      return { wasBudgetSaved: Boolean(response), success, warning };
    } catch (error) {
      return rejectWithValue(error);
    }
  }
