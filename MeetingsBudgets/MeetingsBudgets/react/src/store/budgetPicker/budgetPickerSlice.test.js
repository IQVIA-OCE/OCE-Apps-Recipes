...
describe('budgetPickerSlice', () => {
  describe('asyncThunk', () => {
    ...
    describe('saveBudget asyncThunk', () => {
      test('should save budget and remove it from budgets array in store and return success message', async () => {
        budgetsApi.saveBudget.mockResolvedValueOnce([
          ['some_id_from_API'],
        ...
        const result = await store.dispatch(saveBudget(MAPPED_BUDGETS_DATA[0]));
      ...
      test('should save budget and remove it from budgets array in store and return warning message', async () => {
        budgetsApi.saveBudget.mockResolvedValueOnce([
          ['some_id_from_API'],
          {
            isNotSupported: [{ id: 1 }, { id: 2 }],
            isTooLong: [{ id: 1 }, { id: 2 }],
          },
        ]);
        ...
        expect(result.payload).toEqual({
          wasBudgetSaved: true,
          success: null,
          warning: 'All changes have been saved, but 4 workflow context conditions were ignored. 2 conditions are not supported. 2 conditions are too long.',
        });
      });
