...
describe('Query generator', () => {
  ...
  it('should add offset', () => {
    const query = new SOQL('Account').limit(10).offset(10).build();
    expect(query).toBe(`SELECT Id FROM Account LIMIT 10 OFFSET 10`);
  });
  ...
  it('should add where condition', () => {
    const query = new SOQL('Account')
      ...
      .build();
    expect(query).toBe(
      `SELECT Id FROM Account WHERE Id = '001E000001KnMkTIAV'`
    );
  });
});
