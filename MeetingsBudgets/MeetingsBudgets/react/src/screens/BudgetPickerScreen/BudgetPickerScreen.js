import { ApolloProgress, Card, Portal, Text, Title, Tooltip, useTheme } from '@oce-apps/apollo-react-native';
...
export const BudgetPickerScreen = ({ parentId }) => {
  ...
  if (isBootstrapping || isSubmitting) {
    return (
      <View
        style={[styles.bigLoaderWrapper, { backgroundColor: color(theme.colors.surface).alpha(0.8).string() }]}
        testID={'apolloProgress'}
      >
        <ApolloProgress />
      </View>
    );
