import { ApolloProgress, Caption, Title, useTheme } from 'apollo-react-native';
...
import { getDataToDisplay, isIphone, isWeb } from '../utils';
...
export const TerritoryStatusScreen = ({ ...props }) => {
  ...
  return (
    <View {...props} style={isWeb ? [styles.wrapperWeb, { backgroundColor: theme.colors.surface }] : styles.wrapperIOS}>
      <View
        style={isIphone ? styles.titleContainerVerticalLayout : styles.titleContainerHorizontalLayout}
      >
        <Title style={styles.title}>Territory Status</Title>
        <Caption
          style={{
            marginLeft: isIphone ? 0 : CAPTION_MARGIN,
      ...
      <View
        style={isIphone ? styles.groupsContainerVerticalLayout : styles.groupsContainerHorizontalLayout}
      >
        {groupsList.map(
          ({ name, subgroups }, index) => (
            <Group
              ...
              subgroups={subgroups}
              style={{
                width: isIphone ? '100%' : `${100 / groupsList.length}%`,
                borderRightWidth: index < groupsList.length - 1 && !isIphone ? GROUP_BORDER_WIDTH : 0,
                borderBottomWidth: index < groupsList.length - 1 && isIphone ? GROUP_BORDER_WIDTH : 0,
                paddingLeft: isIphone || index === 0 ? 0 : GROUP_PADDING,
                paddingRight: isIphone || index === groupsList.length - 1 ? 0 : GROUP_PADDING,
              }}
            />
          ),
        )}
