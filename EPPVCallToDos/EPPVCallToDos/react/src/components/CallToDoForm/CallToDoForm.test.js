...
import { Provider, RadioButton, Select, TextInput } from 'apollo-react-native';
...
describe('CallToDoForm', () => {
  ...
  it('should render properly if call account not HCO or Pharmacy and handle form actions', async () => {
    ...
    const { getByTestId, container, update } = render(
      <Provider>
        <CallToDoForm />
      </Provider>
    );
    ...
    const complianceTypeRadioButtonGroupEl = container.findByType(RadioButton.Group);
