...
import { Button, Menu, Provider } from 'apollo-react-native';
...
describe('CallToDosTable', () => {
  ...
  describe('actions', () => {
    ...
    it('edit', async () => {
      const { container, getByTestId, UNSAFE_getAllByType } = render(
        <Provider>
          <CallToDosTable callId={'1'} />
        </Provider>
      );

      const firstActionsIcon = container.findAllByProps({ icon: 'dots-vertical' })[0];
      act(() => {
    ...
    it('delete', async () => {
      const { container, UNSAFE_getAllByType } = render(
        <Provider>
          <CallToDosTable callId={'1'} />
        </Provider>
      );
