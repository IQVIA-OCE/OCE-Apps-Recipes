...
import { ActivityIndicator, Colors, BarChart, Button, IconButton } from "apollo-react-native";
...
export default class StackedBarChartCallDetails extends React.Component {
    ...
    fetchCalldata = () => {
        ...
        return new Promise((resolve, reject) => {
            sfNetAPI
                .query(`SELECT OCE__CallType__c, OCE__CallDateTime__c FROM OCE__Call__c WHERE OCE__CallDateTime__c != null ${filter} ORDER BY OCE__CallDateTime__c`,
                    data => {
                        let chartData = [];
                        ...
                        if (data.records.length){
                            minDate = `${data.records[0].OCE__CallDateTime__c.substring(0, 8)}01`;
                            maxDate = `${data.records[data.records.length-1].OCE__CallDateTime__c.substring(0, 8)}01`;
                        }

                        Object.keys(callDataByType).map(callType => {
                            let groupedByDate = callDataByType[callType].reduce((r, {OCE__CallDateTime__c}) => {
                                if (OCE__CallDateTime__c) {
                                    let formatedCallDateTime = `${OCE__CallDateTime__c.substring(0, 8)}01`;

                                    if(!r[formatedCallDateTime]) r[formatedCallDateTime] = {x: new Date(formatedCallDateTime), y: 1}
                                    else r[formatedCallDateTime].y++;
                                }
                                return r;
                            }, {});
                            const callValues = Object.keys(groupedByDate).map( (key) => groupedByDate[key]);
    ...
    render() {
        ...
        if (minDate){
            const zoomDomainMin = moment(new Date(minDate)).subtract(1, 'M');
            const zoomDomainMax = moment(new Date(minDate)).add(1, 'Y').subtract(1, 'M');
            ...
            <View>
                <View style={styles.topButtons}>
                    {connectionStatus != 'No Connection' ? <View style={styles.refreshButton}><IconButton
                            ...
                            disabled={loading}
                        /></View> : null}
                    <Button style={styles.buttonText} icon="phone" mode="outlined" onPress={() => this.logACall(recordTypeId)}>Call</Button>
                </View>
                ...
                
                {
                    ...
                    <BarChart
                        ...
                        minDomain={{ x: moment(new Date(minDate)).subtract(1, 'M') }}
                        tickFormatX={(x) => moment(x).format('MMM, YY')}
                        tickCount={12}
                        hasAxis={true}
                    /> : null 
                }
