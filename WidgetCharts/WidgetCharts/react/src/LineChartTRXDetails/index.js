    ...
    try {
      ...
      if (data.records) {
        return data.records.map((item) => {
          const periodLabels = [
            item.OCE__PeriodLabelBucket24__c,
            item.OCE__PeriodLabelBucket23__c,
            item.OCE__PeriodLabelBucket22__c,
            item.OCE__PeriodLabelBucket21__c,
            item.OCE__PeriodLabelBucket20__c,
            item.OCE__PeriodLabelBucket19__c,
            item.OCE__PeriodLabelBucket18__c,
            item.OCE__PeriodLabelBucket17__c,
            item.OCE__PeriodLabelBucket16__c,
          ];

          const measures = [
            item.OCE__Measure1Bucket24__c,
            item.OCE__Measure1Bucket23__c,
            item.OCE__Measure1Bucket22__c,
            item.OCE__Measure1Bucket21__c,
            item.OCE__Measure1Bucket20__c,
            item.OCE__Measure1Bucket19__c,
            item.OCE__Measure1Bucket18__c,
            item.OCE__Measure1Bucket17__c,
            item.OCE__Measure1Bucket16__c,
          ];

          return {
            name: item.OCE__Product__r ? item.OCE__Product__r.Name : '',
            data: periodLabels.some(label => label === null)
              ? []
