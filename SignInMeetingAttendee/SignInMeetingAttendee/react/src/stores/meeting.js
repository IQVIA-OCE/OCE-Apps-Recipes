...
export const makeSlice = initialState => createSlice({
  ...
  reducers: {
    ...
    setApplicationLock: (state, action) =>
      Object.assign({}, state, {
        isApplicationLock: action.payload,
      }),
  }
});
      ...
      if (data.records[0]) {
        ...
        const updateArr = await Promise.all(meetingMembersInfo && meetingMembersInfo.records.map(async (member, index) => {
          ...
          if (networkStatusString !== 'No Connection') {
            ...
            if (recordTypeDescribe) {
              [recordTypeDescribe] = recordTypeDescribe.detailLayoutSections.filter((detailLayout) => detailLayout.heading === "Information");
              if (recordTypeDescribe) {
                recordTypeDescribe.layoutRows.forEach((layout) => {
                  const [findItem] = layout.layoutItems.filter((items) => items.label === "Discrepancy Reason");
                  if (findItem) {
                    [discrepancyReasons] = findItem.layoutComponents[0].details.picklistValues.filter((val) => val.label === 'Meeting contains restricted products');
                  }
                });
              }
            }
...
export function fetchLayoutsAsync(objType) {
  return async function (dispatch) {
    try {
      const meetingLayouts = await fetchLayouts(objType);
      dispatch(setMeetingLayouts(meetingLayouts[0]));
    } catch (err) {
    }
  };
}
