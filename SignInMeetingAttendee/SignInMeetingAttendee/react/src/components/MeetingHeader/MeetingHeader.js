...
import { Paragraph, useTheme } from '@oce-apps/apollo-react-native'
...
const MeetingHeader = () => {
    ...
    const renderHightLightPanel = () => {
        return highlightPanel && highlightPanel.filter((filelds) => filelds.layoutComponents[0].value !== 'Name').map((field, index) => {
            ...
            return (
                <View key={index} style={styles.layoutContainer}>
                    <View style={index === highlightPanel.length - 2 ? styles.row : [styles.meetingInfo, { borderColor: theme.dark ? color(theme.colors.surface).lighten(1.5).hex() : color(theme.colors.surface).darken(1.5).hex() }]}>
