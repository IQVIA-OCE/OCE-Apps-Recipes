...
import { environment, customSettings } from 'oce-apps-bridges';
import {
...
import { normalizer, lowercaseKeys } from '../../utils/utils';
...
const SampleOrder = () => {
  ...
  let [userProfile] = useFetcher(
    ...
    normalizer({ [`${NAMESPACE}ProfileId__c`]: 'profileId' })
  );

  useEffect(() => {
    if (userProfile.data && userProfile.data.length && !userProfile.loading) {
  ...
  const getConfigs = async () => {
    try {
      const configData = await customSettings.getCustomSetting(`${NAMESPACE}SamplesManagemenConfig__c`);

      const [config] = normalizer(configKeys)([
        lowercaseKeys(configData)
      ]);

      setValue(prevState => ({
        ...prevState,
        sampleOrderConfig: config,
      }));
    } catch (error) {
      setBanner({
        variant: 'warning',
  ...
  const containerHeight = Platform.OS === 'web' ? 600 : '100%';
  ...
  return (
    <KeyboardAwareScrollView
      ...
      style={[styles.root, { backgroundColor: theme.colors.background, height: containerHeight }]}
      testID="SampleOrder"
    >
      <Formik
        enableReinitialize
