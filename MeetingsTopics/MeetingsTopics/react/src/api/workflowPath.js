...
export const fetchContextType = object__c => {
  return api.queryOffline(
    `SELECT Id, ${NAMESPACE}Object__c, ${NAMESPACE}isControlledByWorkflowPath__c FROM ${NAMESPACE}ContextType__mdt WHERE ${NAMESPACE}Object__c = '${object__c}' AND ${NAMESPACE}Relationship__c = null AND ${NAMESPACE}isControlledByWorkflowPath__c = 1`
  );
};
