...
export const fetchOrderDetails = (
  ...
  return databaseManager.fetch(
    `SELECT Id, Name, ${NAMESPACE}Account__c, ${NAMESPACE}Free__c, ${NAMESPACE}OrderDelivery2__c, ${NAMESPACE}OrderDelivery2__r.${NAMESPACE}DeliveryDate__c,\
        ...
        WHERE ${NAMESPACE}OrderDelivery2__r.${NAMESPACE}DeliveryDate__c >= ${fromDateTimeUtc} AND ${NAMESPACE}OrderDelivery2__r.${NAMESPACE}Status__c NOT IN ('Cancelled') AND ${NAMESPACE}OrderDelivery2__r.${NAMESPACE}Order2__r.${NAMESPACE}Status__c = 'Submitted' ${idFilter} ${productNameFilter} ${brandFilter} AND ${NAMESPACE}Product__r.RecordType.DeveloperName != 'Hierarchy'\
