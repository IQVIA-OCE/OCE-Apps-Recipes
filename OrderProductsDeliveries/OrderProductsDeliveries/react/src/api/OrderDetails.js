...
export const fetchOrderDetails = (
  ...
  if (recordType == RecordType.Order) {
    idFilter = `AND ${NAMESPACE}OrderDelivery2__r.${NAMESPACE}Order2__c = '${id}'`;
  }
  if (recordType == RecordType.Account || recordType == null) {
...
export const fetchOrderById = (id) => {
  return databaseManager.fetch(
    `SELECT Id, Name FROM ${NAMESPACE}Order2__c WHERE Id = '${id}'`
  );
};
