...
import { themePrimary, Text, useTheme } from 'apollo-react-native';
...
export const AccrodianHeader = ({ title, expanded, filterCount }) => {
    ...
    return (
        <View style={styles.filterButton} testID={'accordianHeader'}>
            <Text style={[styles.header, { color: theme.dark ? theme.colors.primary : themePrimary[500] }]} testID={'accordianTitle'}>{title}</Text>
