...
export const AccordianiPhoneContent = () => {
    ...
    const onChangeInputText = (val = '', key) => {
        let obj = inputTextValue;
        obj[key] = val;
        setInputTextValue({ ...obj, ...inputTextValue });
    }
    const onChangeAccessLevel = (val) => {
        const obj = {}
        obj['Territory2.AccountAccessLevel'] = '';
        if (val) {
            obj['Territory2.AccountAccessLevel'] = val.value;
        }
            ...
            <View style={[styles.row, styles.mt10]}>
                ...
                <View style={styles.flex}>
                    <Select
                        ...
                        onChange={(val) => onChangeAccessLevel(val)}
                        style={styles.select}
