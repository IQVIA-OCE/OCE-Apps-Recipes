        ...
        <View style={styles.row}>
            <TouchableOpacity testID={'sortListLink'} onPress={onTapSortList}>
                <View style={styles.linkContainer}>
                    <View><Text style={styles.linkText} testID={'sortText'} numberOfLines={1}>Sort by: {sortOption?.label}</Text></View>
                </View>
            </TouchableOpacity>
            {shouldShowSortList && <SortList testID={'sortList'} />}
            <TouchableOpacity onPress={onToggleSort} testID={'sortListIcon'}>
                <Icon name={sortAscOrder ? 'arrow-up' : 'arrow-down'} size={18} color={themePrimary[500]} />
            </TouchableOpacity>
        </View>
    );
};
