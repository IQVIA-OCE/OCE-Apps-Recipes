...
export default class OneKeyValidationRequest extends React.Component {
  ...
  handleSubmit = async (values, { setSubmitting }, userID) => {
  ...
  submitOneKeyRequest = requestData => {
    return fetch('', { //Provide ONE KEY URL
      method: 'POST',
      credentials: 'include',
      mode: 'no-cors',
      headers: {
  ...
  render() {
    const { picklists, isLoading, loadingMessage, requiredFields, showRequestSubmittedMessage,
      showRequestErrorMessage, countries, entityTypes, userID } = this.state;
          ...
          <SafeAreaView>
            <Formik
              ...
              onSubmit={(values, actions) => {
                this.handleSubmit(values, actions, userID)
              }}
            >
              {({
                handleChange,
