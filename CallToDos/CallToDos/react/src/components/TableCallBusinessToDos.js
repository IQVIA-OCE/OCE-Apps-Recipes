...
export const CustomHeader = ({ onBulkEdit, onBulkDelete, selectedRows, canAdd, canEdit, canDelete, handleOpenBusinessToDos }) => {
  let menuItems = [];
  const menuItemEdit = {
    text: 'Edit',
    onPress: onBulkEdit,
  ...
  return (
    <View style={{flex: 1, flexDirection: 'row-reverse'}}>
      <Button
        ...
        style={styles.button}
        mode="contained"
        onPress={() => handleOpenBusinessToDos(true)}
        disabled={!canAdd}
      >
        Add
      </Button>
...
export const TableCallBusinessToDos = (props) => {
  const {buttonHeaders, handleOpenBusinessToDos, isTestCBT, refreshing} = props;
  ...
  const fetchData = async () => {
    let query = `select Id, OCE__BusinessToDo__c, OCE__BusinessToDo__r.Name, OCE__BusinessToDo__r.OCE__BusinessTactic__r.OCE__BusinessObjective__r.OCE__Product__c, OCE__BusinessToDo__r.OCE__DueDate__c, OCE__Status__c from OCE__CallToDo__c where recordtype.name = 'Call Business To-Do' and OCE__Call__c = '${state.recordId}' ORDER BY CreatedDate DESC`;
    ...
    if (records.length > 0) {
      ...
      records.forEach((node) => {
        node.rowId = i;
        ++i;
        node.name = (node['OCE__BusinessToDo__r.Name'] ? node['OCE__BusinessToDo__r.Name'] : node['OCE__BusinessToDo__r'] ? node['OCE__BusinessToDo__r']['Name'] : '');
        node.OCE__BusinessObjectiveProducts = (node['OCE__BusinessToDo__r.OCE__BusinessTactic__r.OCE__BusinessObjective__r.OCE__Product__c'] ? node['OCE__BusinessToDo__r.OCE__BusinessTactic__r.OCE__BusinessObjective__r.OCE__Product__c'] : node['OCE__BusinessToDo__r'] ? node['OCE__BusinessToDo__r']['OCE__BusinessTactic__r']['OCE__BusinessObjective__r']['OCE__Product__c'] : '');
        node.OCE__DueDate__c = (node['OCE__BusinessToDo__r.OCE__DueDate__c'] ? node['OCE__BusinessToDo__r.OCE__DueDate__c'] : node['OCE__BusinessToDo__r'] ? node['OCE__BusinessToDo__r']['OCE__DueDate__c'] : '');
        node.rowStatus = { label: node.OCE__Status__c, id: node.OCE__Status__c }
        //node.OCE__BusinessTactic = (node['OCE__BusinessToDo__r.OCE__BusinessTactic__r.Name'] ? node['OCE__BusinessToDo__r.OCE__BusinessTactic__r.Name'] : node['OCE__BusinessToDo__r'] ? node['OCE__BusinessToDo__r']['OCE__BusinessTactic__r']['Name'] : null);
      });
      setRows(records);
    }
  };
  ...
  const columns = [
    ...
    {
      header: 'Call Business ToDo Id',
      accessor: 'Id',
      sortFunction: sortStrings,
      hidden: true
    },
    {
      header: 'Business ToDo Id',
      accessor: 'OCE__BusinessToDo__c',
      sortFunction: sortStrings,
      hidden: true
    },
    {
  ...
  return (
    <View>
      <Table
        ...
        headerProps={{ onBulkEdit, onBulkDelete, selectedRows, canAdd, canEdit, canDelete, handleOpenBusinessToDos }}
      />
      <Portal>
        <Modal
      ...
      <View>
        <Button
          testID='buttonFuncs-testCBT'
          title="buttonFuncs-testCBT"
          onPress={() => testFuncsCBT()}
        />
      </View>
    }
    </View>
