...
export const SearchToDo = () => {
    ...
    const fields = [
        ...
        { label: 'Assigned To', value: 'OCE__AssignedTo__r.Name' },
        { label: 'Account', value: 'OCE__Account__r.Name' },
    ...
    const setFiltersByDefault = () => {
        ...
        {
            ++keyNum;
            titleText = `Account Equals ${state.accountName}`;
            filterText = ` or OCE__Account__r.Name = '${state.accountName}'`;
            addedChip = {
                key: keyNum,
                title: titleText,
                filter: filterText
            ...
            if (state.userFullName && state.userFullName.length > 0)
            {
                ++keyNum;
                titleText = `Or Assigned To Equals ${state.userFullName}`;
                filterText = ` or OCE__AssignedTo__r.Name = '${state.userFullName}'`;
                addedChip = {
                    key: keyNum,
                    title: titleText,
                    filter: filterText
            ...
            {
                state.attendees.forEach((node) => {
                    ++keyNum;
                    
                    titleText = `Account Equals ${node}`;
                    if (keyNum > 1)
                        titleText = `Or Account Equals ${node}`;
                    filterText = ` or OCE__Account__r.Name = '${node}'`;
                    addedChip = {
                        key: keyNum,
                        title: titleText,
                        filter: filterText
    ...
    useEffect(() => {
        if (state.setDefaultFiltersToDo)
        {
            setFiltersByDefault();
        }
      }, [state]);
