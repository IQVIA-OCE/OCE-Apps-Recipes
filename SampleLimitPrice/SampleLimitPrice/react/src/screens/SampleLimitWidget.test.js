...
import { Autocomplete } from "@oce-apps/apollo-react-native";
...
describe("SampleLimitWidget", () => {
  ...
  it("SampleLimitWidget should render properly", async () => {
    ...
    expect(await findByText(/Will Denzer/i)).toBeTruthy();
  ...
  it("SampleLimitWidget should render properly with error in fetching products records", async () => {
    ...
    const defaultProductNameEl = await waitFor(() => queryByText("Will Denzer"));
  ...
  it("Should search for a value from Account Search component", async () => {
    ...
    await waitFor(() => UNSAFE_getAllByType(Autocomplete)[1].props.onChange(ACCOUNT));

    expect(
      await waitFor(() => UNSAFE_getAllByType(Autocomplete)[1].props.singleSelectValue)
  ...
  it("should select one product from dropdown", async () => {
    ...
    expect(
      await waitFor(() => UNSAFE_getByType(DonutChartContainer).props.selectedProduct)
    ).toStrictEqual({
      label: "Azelastine100MG",
      value: "0016s00000OWox3AAD",
      details: {
