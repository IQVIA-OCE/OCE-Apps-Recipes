...
class AccountItem extends React.Component {
    ...
    render() {
        ...
        const accountAddress = itemData.OCE__Account__r.OCE__PrimaryAccountAddress__r ? itemData.OCE__Account__r.OCE__PrimaryAccountAddress__r.OCE__Address__r.OCE__DisplayAddress__c : '';
        const countryCode = itemData.OCE__Account__r.OCE__CountryCode__c;

        return (
            <Animated.View style={[styles.container, {opacity: this.state.opacity}]}>
                <View style={styles.container}>
                    <View style={[styles.cell, styles.firstCell]}>
                        <Text style={[styles.title, styles.text, styles.textCaption]}>{itemData.OCE__Account__r.Name}</Text>
                    ...
                    {columns.map((col, i) => {
                        return <View style={styles.cell} key={i}>
                            ...
                            <Text style={styles.text}>
                                {col.type ? resolveObjectPath(col.accessor, itemData) + ordinalNumber(resolveObjectPath(col.accessor, itemData)) + '' : resolveObjectPath(col.accessor, itemData)}
                            </Text>
                        </View>
                    })}
                    <View style={styles.actions}>
                        <IconButton
                            ...
                            size={25}
                            animated
                            onPress={() => navigation.push('AccountInfo', { accountName: itemData.OCE__Account__r.Name, countryCode: countryCode })}
                        />
                        <IconButton
                            icon="account"
                            color="#2bb3fe"
                            size={25}
                            animated
                            onPress={() => this.showCustomerDetails(itemData.OCE__Account__r.Id)}
                        />
                        <IconButton
                            icon="phone"
                            color="#2bb3fe"
