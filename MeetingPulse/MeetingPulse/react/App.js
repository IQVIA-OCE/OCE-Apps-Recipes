...
import { Provider as ApolloProvider, DefaultTheme, DarkTheme, Text, utilityNegative } from 'apollo-react-native';
...
const Root = ({ recordId }) => {
  ...
  const renderContent = () => {
    if (meetingLoadingStatus === LOADING_STATUS.FAILED) {
      return (
        <View style={styles.error} testID="inquiries-error">
          <Text style={[styles.errorText, { color: theme.colors.error }]}>{meetingError}</Text>
        </View>
      );
    }
    if (meetingId) {
