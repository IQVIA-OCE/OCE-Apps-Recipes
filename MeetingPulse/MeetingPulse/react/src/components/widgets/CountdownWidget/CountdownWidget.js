...
const ConflictHint = () => {
  ...
  return (
    <View style={{ justifyContent: 'center', alignItems: 'center', paddingVertical: 8 }} testID="conflict-hint">
      <Icon style={{ marginBottom: 8 }} size={22} name="alert" color={theme.colors.text} />
...
export const CountdownWidget = () => {
  ...
  const startDate = DateTime.fromISO(meeting.startDate, { zone: 'utc' }).toFormat('x');
  const endDate = DateTime.fromISO(meeting.endDate, { zone: 'utc' }).toFormat('x');
