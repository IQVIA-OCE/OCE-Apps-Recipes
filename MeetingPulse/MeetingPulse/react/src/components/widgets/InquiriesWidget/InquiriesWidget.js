...
import { Text, utilityNegative } from 'apollo-react-native';
    ...
    if (isFailed && error) {
      return (
        <View style={styles.error} testID="inquiries-error">
          <Text style={styles.errorText}>Error fetching inquiries: {error}</Text>
        </View>
      );
    }
    ...
    if (isLoaded) {
      ...
      return (
        <View style={styles.notFound} testID="inquiries-not-found">
          <Text style={styles.notFoundText}>There are no recent inquiries from your meeting participants</Text>
        </View>
      );
    }
  };
