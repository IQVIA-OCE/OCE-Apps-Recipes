...
export const fetchInquiryQuestions = accountIds => {
  ...
  const query = `SELECT ${NAMESPACE}Inquiry__r.${NAMESPACE}Account__r.Name, ${NAMESPACE}Inquiry__r.${NAMESPACE}Account__r.Id, ${NAMESPACE}Inquiry__r.Name, ${NAMESPACE}Inquiry__r.Id, Id, ${NAMESPACE}Question__c, ${NAMESPACE}Inquiry__c\
    ...
    WHERE ${NAMESPACE}Inquiry__r.${NAMESPACE}Account__c IN (${formattedAccountIds}) AND ((LastModifiedDate >= ${LAST_90_DAYS} OR ${NAMESPACE}offlineLastModifiedDate__c >= ${LAST_90_DAYS}) OR (${NAMESPACE}Inquiry__r.LastModifiedDate >= ${LAST_90_DAYS} OR ${NAMESPACE}Inquiry__r.offlineLastModifiedDate__c >= ${LAST_90_DAYS}))\
