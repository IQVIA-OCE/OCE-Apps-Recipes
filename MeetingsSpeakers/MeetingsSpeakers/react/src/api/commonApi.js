...
export const fetchCurrentContextNode = async ({ config, meeting, meetingMember }) => {
  ...
  const relatedContextTypeId = config[`${NAMESPACE}ContextType__c`];
  ...
  const node = nodes.find(n => n[`${NAMESPACE}ControllingFieldValue__c`] === objectStatus);

  const [relatedContexts] = await workflowApi.fetchContext(node.Id, false, [relatedContextTypeId]);
  ...
  const [[childType]] = await workflowApi.fetchContextType(`${NAMESPACE}MeetingMember__c`);

  if (!childType) return [null, []];

  const [[childWorkflowConfig]] = await workflowApi.fetchWorkflowConfiguration(childType.Id);

  if (!childWorkflowConfig) return [null, []];

  const [childNodes] = await workflowApi.fetchWorkflowNode(childWorkflowConfig.Id);

  const relatedChildNode = childNodes.find(n =>
    n[`${NAMESPACE}ControllingFieldValue__c`] ===
    meetingMember[childWorkflowConfig[`${NAMESPACE}ControllingField__c`]]
  );

  const [childContexts] = await workflowApi.fetchContext(relatedChildNode.Id, true);
