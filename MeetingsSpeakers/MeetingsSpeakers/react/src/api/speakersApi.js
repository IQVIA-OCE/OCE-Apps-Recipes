...
export const fetchMeetingMembers = (meetingId, recordTypeId) => {
  const query = `SELECT Id, Name, RecordType.DeveloperName, ${NAMESPACE}Speaker__c, CreatedDate FROM ${NAMESPACE}MeetingMember__c\
...
export const getDbSearchFields = async () => {
  ...
  const isAdvancedSearchEnabled = await getIsAdvancedSearchEnabled(userID);
