...
import { ApolloProgress, Title, Tooltip, useTheme } from '@oce-apps/apollo-react-native';
import {
...
export const SpeakerPickerScreen = ({ parentId, recordTypeId }) => {
  ...
  const wrapperBgColor = theme.dark ? theme.colors.background : '#ffffff';

  return (
    <View style={{ backgroundColor: wrapperBgColor, flex: 1 }}>
      {isLoading && (
