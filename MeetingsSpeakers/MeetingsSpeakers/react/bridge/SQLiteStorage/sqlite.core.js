  ...
  if (
    ...
    openerrorcb = (function (_this) {
      return function (e) {
        ...
        if (!!error) {
          error(newSQLError('Could not open database, ' + e));
        }
...
SQLiteFactory.prototype.getDatabasesInfo = function (success, error) {
  if (plugin.sqlitePlugin.DEBUG) {
    plugin.log('SQLite.getDatabasesInfo()');
  }
  NativeModules.SQLite.getDatabasesInfo(success, error);
};
