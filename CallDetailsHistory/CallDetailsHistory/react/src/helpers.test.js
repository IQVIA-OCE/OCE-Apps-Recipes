...
describe('helpers', () => {
  ...
  it('getNormalizedChartData should return normalized call data for chart', () => {
    const chartData = getNormalizedChartData(NORMALIZED_DATA, 6);

    expect(chartData.length).toEqual(2);
    expect(chartData[0].data.length).toEqual(6);
    expect(chartData[1].data.length).toEqual(6);
    expect(chartData[0].data[4].tooltip).toBe('A:Maxton: Submitted 1 times');
    expect(chartData[1].data[5].tooltip).toBe('B:Alodox: Submitted 1 times');
  });
});
