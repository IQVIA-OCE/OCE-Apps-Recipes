...
import { IconButton, LineChart, Menu, Provider, Search } from 'apollo-react-native';
...
describe('CallDetailsHistory', () => {
  ...
  it('should render chart if component gets all needed data', async () => {
    ...
    const { container } = render(<CallDetailsHistory recordId={'0'} />);

    await waitFor(() => {
      expect(container.findByType(Search).props.value).toBe(ACCOUNTS_MOCK[0].Name);
      expect(container.findByType(LineChart)).toBeTruthy();
    });
  });
  ...
  it('should open list of accounts when it is searching', async () => {
    ...
    await waitFor(() => {
      expect(container.findAllByType(Menu.Item).length).toEqual(0);
    });

    await waitFor(() => {
      container.findByType(Search).props.onFocus();
    });

    await waitFor(() => {
      container.findByType(Search).props.onChangeText('test');
    });

    await waitFor(() => {
      expect(container.findAllByType(Menu.Item).length).toEqual(2);
  ...
  it('should close list of accounts', async () => {
    ...
    await waitFor(() => {
      expect(container.findAllByType(Menu)[0].props.visible).toBeTruthy();
    });

    act(() => {
      container.findAllByType(Menu)[0].props.onDismiss();
    });

    await waitFor(() => {
      expect(container.findAllByType(Menu)[0].props.visible).toBeFalsy();
    });
  });
  ...
  it('should change account name in search input if accounts was selected', async () => {
    ...
    await waitFor(() => {
      expect(container.findByType(Search).props.value).toBe('');
      expect(container.findAllByType(Menu.Item).length).toEqual(0);
    });
    ...
    await waitFor(() => {
      container.findAllByType(Menu.Item)[1].props.onPress();
    });

    await waitFor(() => {
      expect(container.findAllByType(Menu.Item).length).toEqual(0);
      expect(container.findByType(Search).props.value).toBe('David Labotka');
    });
  });
  ...
  it('should start refreshing if refresh button was pressed', async () => {
    ...
    act(() => {
      container.findAllByType(IconButton).find(el => el.props.icon === 'refresh').props.onPress();
    });

    await waitFor(() => {
      expect(consoleWarnSpyOn).toHaveBeenCalled();
      expect(getNormalizedData.mock.calls.length).toEqual(2);
      expect(container.findByType(Search).props.value).toBeUndefined();
    });
  });
});
