...
const fetchAccountCalls = async accountId => {
  const [accountCalls] = await api.query(`SELECT Id, ${NAMESPACE}CallDateTime__c FROM ${NAMESPACE}Call__c WHERE ${NAMESPACE}Account__c = '${accountId}' AND ${NAMESPACE}Status__c = 'Submitted'`);

  return accountCalls;
};

const fetchCallDetails = async accountCalls => {
  const callsQueryString = accountCalls.map(el => `${NAMESPACE}Call__c = '${el.Id}'`).join(' OR ');
  const [callDetails] = await api.query(`SELECT Id, ${NAMESPACE}Call__c, ${NAMESPACE}CallDateTime__c, ${NAMESPACE}Product__r.Name, ${NAMESPACE}Product__r.Id FROM ${NAMESPACE}CallDetail__c WHERE ${callsQueryString}`);
